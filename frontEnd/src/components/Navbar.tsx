...
const Navbar: React.FC = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      await axios.post("https://calorierose.vercel.app/api/logout");
  ...
  return (
    <>
      ...
      <button
        ...
        className={`text-black focus:outline-none z-50 absolute top-4 left-5 ${
          !isOpen ? "moveup" : ""
        }`}
      >
        <svg
      ...
      <nav
        className={`bg-gray-900 text-white  lg:w-[24vw] min-h-screen lg:block flex flex-col ${
      ...
      >
        ...
        <div className={`lg:hidden ${isOpen ? "block" : "hidden"} bg-gray-900`}>
          <ul className="flex flex-col gap-2 p-4 fixed">
            <Link to="/dash" onClick={() => setIsOpen(false)}>
              <li
                className={`px-5 py-3 text-lg border-l-4 ${
                  path === "" ? "border-blue-500" : "border-transparent"
                } hover:bg-gray-800`}
              >
                Dashboard
              </li>
            ...
            >
              <li
                className={`px-5 py-3 text-lg border-l-4 ${
                  path === "meals" ? "border-blue-500" : "border-transparent"
                } hover:bg-gray-800`}
              >
                Meals
              </li>
            ...
            <Link to="/news" onClick={() => setIsOpen(false)}>
              <li
                className={`px-5 py-3 text-lg border-l-4 ${
                  path === "news" ? "border-blue-500" : "border-transparent"
                } hover:bg-gray-800`}
              >
                News
              </li>
            ...
            <Link to="/tools" onClick={() => setIsOpen(false)}>
              <li
                className={`px-5 py-3 text-lg border-l-4 ${
                  path === "tools" ? "border-blue-500" : "border-transparent"
                } hover:bg-gray-800`}
              >
                Tools
              </li>
            ...
            <li
              className="px-5 py-3 text-lg border-l-4 border-transparent hover:bg-gray-800 cursor-pointer"
        ...
        <div
          className={`hidden lg:flex flex-col gap-2 p-4 ${
            isOpen ? "hidden" : ""
          }`}
        >
